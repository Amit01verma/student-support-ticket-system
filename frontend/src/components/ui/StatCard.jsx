export default function StatCard({ label, value, accent = "teal", icon }) {
  const accents = {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/40">
      <div className="flex items-start justify-between gap-3">
        ...
        <span
          className={`flex h-9 w-9 items-center justify-center rounded-xl text-sm font-bold ${accents[accent]}`}
        >
          {icon}
        </span>
      </div>
